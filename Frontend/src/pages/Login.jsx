    ...
    return (
        <div className='w-full h-full flex items-center justify-center bg-white'>
            <div className="w-full h-full flex flex-col items-center justify-center p-5">
                <h1 className='text-2xl font-bold underline underline-offset-4 mb-6'>Login to Expense Tracker</h1>
                <form onSubmit={handleSubmit} className="w-[80%] h-auto flex flex-col gap-8 items-center p-5">
                    ...
                    <button disabled={!isFormValid} className={`bg-[#ffc72e] ${isFormValid ? 'bg-[#ffc72e] cursor-pointer' : 'bg-[#f3ce6a] cursor-not-allowed'} w-full py-4 text-[#222222] text-xl font-semibold rounded-md mt-4`}>Login</button>
                    <Link to={'/'} className='text-lg font-semibold'>Don't have an account? <span className='text-gray-800 font-medium'>Signup</span></Link>
                </form>
            </div>
            <div className="w-[80%] h-[80%">
                <img src="./images/login-illustration.png" alt="" className='w-full h-full object-cover' />
            </div>
        </div>
    )
}
